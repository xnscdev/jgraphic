...
public class RectButton extends Gui {
    ...
    public RectButton(String label, Vector2f position, Vector2f size, Vector4f backgroundColor, Runnable callback) {
        super(GuiManager.RECT_MODEL, position, size, backgroundColor);
        this.callback = callback;
        float fontSize = size.y * 0.6f;
        labelText = new GuiText(label, fontSize, Fonts.PLAY, new Vector2f(0, size.y * 0.2f), size.x, true);
        addChild(labelText);
    }

    public RectButton(String label, Vector2f position, Vector2f size, String texture, Runnable callback) {
